...
const App = () => {
    ...
    const handleAddModalOk = () => {
        form
            ...
            .then(async (values) => {
                try {
                    const response = await axios.post('http://localhost:8080/api/terms', {
    ...
    const handleUpdate = async () => {
        try {
            const response = await axios.patch(`http://localhost:8080/api/terms/${termNo}`, {
    ...
    const handleDelete = async () => {
        Modal.confirm({
            ...
            onOk: async () => {
                try {
                    // API를 통해 삭제 요청
                    const response = await axios.delete(`http://localhost:8080/api/terms/${selectedTerm}`);
